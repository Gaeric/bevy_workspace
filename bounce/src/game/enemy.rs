...
pub fn move_enemy(
    ...
    for (enemy, controller, mut motion) in query.iter_mut() {
        let velocity = controller.map(|controller| controller.velocity).unwrap_or_default();
        motion.velocity = motion.velocity.damp(velocity, enemy.damp, time.delta_seconds() * time_scale.0).clamp_length_max(enemy.max_speed);
    }
}
                ...
                && position.y > enemy.hit_height_threshold
            {
                ...
                if let Some(candidate) = trajectory
                    ...
                    .min_by(|a, b| {
                        ...
                        if distance < ENEMY_BREAKE_DISTANCE {
                            speed *= distance / ENEMY_BREAKE_DISTANCE;
                        }
                        speed * direction.normalize()
                    } else {
                        let collection: Vec<_> = trajectory.points.iter().map(|point| point.position).filter(|position| position.y > -ARENA_HEIGHT / 2.0).map(
                            |position| {
                        ...
                        candidate.y = candidate.y.clamp(0.125 * ARENA_HEIGHT, 0.375 * ARENA_HEIGHT);

                        let direction = candidate - position;
                        let distance = direction.length();
                        let speed = if distance < ENEMY_BREAKE_DISTANCE {
                            if motion.translation.y < 0.0 {
                                0.0
                            } else {
                                enemy.normal_speed * distance / ENEMY_BREAKE_DISTANCE
                            }
                        } else {
                            enemy.normal_speed
                        };
