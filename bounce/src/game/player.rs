...
pub fn move_player(
    ...
    let delta = mouse_motion_events.iter().map(|mouse_motion| mouse_motion.delta).map(|v| Vec2::new(v.x, -v.y)).fold(Vec2::ZERO, Vec2::add);
    ...
    for (player, controller, mut motion_override, mut motion) in query.iter_mut() {
        ...
        motion.velocity = motion.velocity.damp(velocity, damp, delta_seconds).clamp_length_max(player.max_speed);
    }
}
...
pub fn assist_player(
    ...
    for (transform, assist, mut controller, _) in query.iter_mut() {
        ...
        for (motion, trajectory) in ball_query.iter() {
            ...
            if motion.velocity.y < assist.vertical_speed_threshold && motion.velocity.length() > assist.speed_threshold && delta.x.abs() > assist.range {
                let delta_seconds = time.seconds_since_startup() - trajectory.start_time;
                if let Some(candidate) = trajectory.points.iter().filter(|point| point.position.y < 0.0)
            ...
            time_scale.0 = time_scale.0.damp(target_time_scale, TIME_SCALE_DAMP, time.delta_seconds());
