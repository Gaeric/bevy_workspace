...
fn death_effect_system(
    ...
    for (entity, mut transform, mut effect) in query.iter_mut() {
        if effect.timer.tick(Duration::from_secs_f32(time.delta_seconds() * time_scale.0)).just_finished() {
...
fn hit_effect_system(
    ...
    for (entity, mut effect, mut sprite, texture_atlas_handle) in query.iter_mut() {
        if effect.timer.tick(Duration::from_secs_f32(time.delta_seconds() * time_scale.0))
            .just_finished() {
